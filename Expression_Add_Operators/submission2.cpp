...
public:
    ...
    void solve(string &num, int target, int index, int n, string &temp, vector<string> &ans, long long current, long long prev, int sign){
        ...
        if (index==0){
            ...
            if (num[index] == '0'){
                nextNum = nextNum*10 + num[index]-'0';
                temp.push_back(num[index]);
                solve(num, target, index+1, n, temp, ans, nextNum, nextNum, 1);
                temp.pop_back();
            } else {
                for (int i=0; i<n; i++){
            ...
            for (auto op : operators){
                ...
                if (num[index] == '0'){
                    ...
                    temp.push_back(num[index]);
                    if (op == '+'){
                        solve(num, target, index+1, n, temp, ans, current+nextNum, nextNum, 1);
                    } else if (op == '-'){
                        solve(num, target, index+1, n, temp, ans, current-nextNum, nextNum, 0);
                    } else {
                        if (sign == 0){
                            solve(num, target, index+1, n, temp, ans, (current+prev) - (prev*nextNum), prev*nextNum, 0);
                        } else if (sign == 1){
                            solve(num, target, index+1, n, temp, ans, (current-prev) + (prev*nextNum), prev*nextNum, 1);
                        } else {
                            solve(num, target, index+1, n, temp, ans, current*nextNum, prev*nextNum, 2);
                        }
                    }
